...
def dashboard(request):
    ...
    if asesor_autenticado:
        if request.method == "POST":
            tipo = request.POST.get("tipo")  # Asegurar que coincida con el select
...
def obtener_clientes_actualizados(request):
    try:
        hoy = timezone.now().date()  # Obtiene la fecha actual con zona horaria
        ventas = Venta.objects.filter(fecha_hora__date=hoy).values(
            "dni_cliente", "cliente", "telefono", "distrito", 
            "IdAsesor__nombre_apellido"
        ).annotate(fecha=F("fecha_hora__date"))  # Extrae solo la fecha
        
        print("Ventas obtenidas:", list(ventas))  # Debug
...
def obtener_clientes(request):
    try:
        hoy = timezone.now().date()  # Fecha actual con zona horaria
        clientes = Cliente.objects.filter(fecha_hora__date=hoy).values(
            "dni_cliente", "nombre", "telefono", 
            "tipificacion", "descripcion", "IdAsesor__nombre_apellido"
        ).annotate(fecha=F("fecha_hora__date"))  # Extrae solo la fecha
        
        print("Clientes obtenidos:", list(clientes))  # Debug
